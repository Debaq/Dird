...
interface AnnotationsCanvasLayerProps {
  // AI Detections
  aiDetectionsVisible: boolean;
  aiDetectionsOpacity: number;
  detections: any[];
  segmentations: any[];

  // Manual Annotations
  manualAnnotationsVisible: boolean;
  manualAnnotationsOpacity: number;
...
export const AnnotationsCanvasLayer: React.FC<AnnotationsCanvasLayerProps> = ({
  aiDetectionsVisible,
  aiDetectionsOpacity,
  detections,
  segmentations,
  manualAnnotationsVisible,
  manualAnnotationsOpacity,
  manualAnnotations,
  manualSegmentations,
  segmentationImages,
  konvaImageWidth,
  ...
  const renderDetectionBox = (detection: any, idx: number, isManual: boolean) => {
    ...
    const labelText = isManual
      ? translatedClass
      : `${translatedClass} ${Math.round((detection.confidence || 0) * 100)}%`;
    ...
    const showLabels = isManual ? showManualLabels : showAILabels;

    return (
      <Group key={`${isManual ? 'manual' : 'ai'}-${detection.id || idx}`}>
        <Rect
          id={`det-${detection.id}`}
          ...
          fill={`${detectionColor}${isManual ? '20' : '1A'}`}
          stroke={isHovered ? "#FFD700" : detectionColor}
          strokeWidth={isHovered ? 3 : (isManual ? 1 : 1.5)}
          dash={isHovered ? [] : (isManual ? [4, 2] : [8, 4])}
          hitStrokeWidth={10}
          perfectDrawEnabled={false}
          listening={activeTool !== 'bbox'}
          opacity={isLandmarkClass && !isSelected ? 0 : 1}
        ...
        {isLandmarkClass && (() => {
          const landmark = landmarks.find(l => {
            const match = l.id.match(/(?:manual|ai)-.*-(\d+)/);
  ...
  return (
    ...
    >
      ...
      {isCanvasReady && detections.length === 0 && manualAnnotations.length === 0 && (
        <Text
          x={imageOffset.x + 20}
          y={imageOffset.y + 20}
      ...
      {/* AI Segmentations (optic disc masks) - rendered first */}
      {aiDetectionsVisible && segmentations.map((seg) => {
        ...
        return (
          <KonvaImage
            ...
            scaleY={scale}
            listening={false}
            opacity={0.4 * aiDetectionsOpacity}
          />
        );
      })}

      {/* Manual Segmentations */}
      {manualAnnotationsVisible && manualSegmentations.map((seg) => {
        const segImage = segmentationImages.get(seg.id!);
        if (!segImage || !seg.visible) return null;
        ...
        return (
          <KonvaImage
            ...
            scaleY={scale}
            listening={false}
            opacity={0.4 * manualAnnotationsOpacity}
          />
        );
      })}

      {/* AI Detections */}
      {isCanvasReady && aiDetectionsVisible && detections.map((detection, idx) =>
        renderDetectionBox(detection, idx, false)
      )}

      {/* Manual Annotations */}
      {isCanvasReady && manualAnnotationsVisible && manualAnnotations.map((annotation, idx) =>
        renderDetectionBox(annotation, idx, true)
      )}
