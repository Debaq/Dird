#!/usr/bin/env python3
"""
Sub-exp 3 (DDR) — IC 95% bootstrap del punto de operación con τ congelados de APTOS.

Resampleo de imágenes con reemplazo (2000 iter, seed 42) sobre el dump
results/02-sweep/raw_detections.csv. Operating point = FROZEN_fpr02 (τ de APTOS).
AUC = libre de umbral (máx score sobre clases de lesión). Determinista.
"""
import json
...
CALIB = BASE / ".." / "experiment-2-aptos" / "results" / "03-perclass" / "perclass_calibration.json"
LESION = [1, 3, 4, 5]
...
def main() -> None:
    ...
    out_dir = BASE / "results" / "04-bootstrap-ci"
    ...
    calib = json.load(open(CALIB.resolve()))
    ...
    for b in range(N_BOOT):
        ...
        m = point_metrics(y_true[idx], y_pred[idx], y_score[idx])
        for k in keys:
            boot[k][b] = m[k]

    result = {
        "experiment": "DDR bootstrap 95% CI (image-resample, percentile)",
        "n_images": int(n), "n_boot": N_BOOT, "seed": SEED,
        "operating_point": "FROZEN_fpr02 (per-class tau frozen from APTOS)",
        "tau_fpr02": {NAMES[c]: float(t) for c, t in zip(LESION, tau)},
        "metrics": {},
    }
    lines = [f"DDR bootstrap 95% CI — {ts}",
             f"N={n}  n_boot={N_BOOT}  seed={SEED}  resample=image-level",
             "Operating point: FROZEN_fpr02 (τ per-clase congelados de APTOS)", "",
             f"{'metric':<14}{'point':>9}{'CI95_low':>11}{'CI95_high':>11}"]
    for k in keys:
        lo, hi = np.percentile(boot[k], [2.5, 97.5])
        result["metrics"][k] = {"point": point[k], "ci95_low": float(lo), "ci95_high": float(hi),
                                "boot_mean": float(boot[k].mean()), "boot_std": float(boot[k].std())}
